...
const Button = ({type = 'button', setWidth = '100%', position = 'left', text = '', showBorder = false, bgcolor, color, isLoading = false, handleAction = null, icon = null, flagFilter = false}) => {
    ...
    return (
        <div className={`py-1`} style={{width: setWidth}}>
            ...
            <button className={`${type === 'inputFile' ? 'rounded-l-md' : 'rounded-md' } p-2 font-bold cursor-pointer ${showBorder ? `border-2 border-[#369D00]` : '' }`} style={{width: setWidth, background: bgcolor, color: color, boxShadow: '0 1px 4px 0 rgba(0, 0, 0, 0.2)'}} onClick={flagFilter ? () => handleAction(text) : handleAction}>
