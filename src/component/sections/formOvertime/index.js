...
const FormOvertime = ({dataObj, isAdd, setIsAdd, isEdit, setIsEdit, listType = [], listEmployee = [], handleChange, handleChangeSelect, handleView, targetDate = null, showForm = false, setWidth = '100%', btnApprove = false, btnCancel = false, btnAction = true, handleAfterExecute, inputLock = false, btnAdd = false}) => {
    ...
    return (
        <InputContent showForm={showForm} setWidth={setWidth} btnLabel="Tambah Lembur" isEdit={isEdit} setIsEdit={setIsEdit} isAdd={isAdd} setIsAdd={setIsAdd} handleAction={handleSubmit} handleApproveReject={handleApproveReject} handleCancel={handleCancel} showBtnApprove={btnApprove} showBtnCancel={btnCancel} showBtnAction={btnAction} showBtnAdd={btnAdd}> 
            {(isAdd || isEdit) ?          
