...
const Select = ({data = [], defaultLabel = '', setWidth = '100%', value, isError = null}) => {
        ...
        <div className="py-2 relative">
            <div className="bg-[#ffffff] rounded-lg flex flex-row items-center mr-2 cursor-pointer" style={{boxShadow: '0 1px 4px 0 rgba(0, 0, 0, 0.2)', userSelect: 'none'}} onClick={() => setOpen(!open)}>
                <div className="p-2 w-[145px]">
                    <p className="text-sm font-semibold">{`${defaultLabel || 'Select Type'}`}</p>
                </div>
                ...
                <div className="absolute top-14 border border-gray-400 bg-[#ffffff] rounded-lg w-[180px]" style={{boxShadow: '0 1px 4px 0 rgba(0, 0, 0, 0.2)'}}>
                    ...
                    {data?.map((val, idx) => (
                        <div key={idx} className="cursor-pointer p-2 border-b border-gray-400 hover:bg-[#ddd]" style={{transition: '.1s'}}>
                            <p className="text-sm font-normal">{val?.value}</p>
                        </div>
                    ))}
                </div>
            }
